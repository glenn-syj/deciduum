...
async def test_today_empty(client: AsyncClient, auth_headers: dict):
    ...
    data = response.json()
    assert data["data"]["ongoing_decisions"] == []
    assert data["data"]["todays_decisions"] == []
    assert data["data"]["todays_memos"] == []
...
async def test_today_ongoing_decisions(client: AsyncClient, auth_headers: dict):
    ...
    await client.post(
        "/v1/decisions",
        json={"title": "Completed Decision", "date": "2026-02-15", "status": "completed"},
        headers=auth_headers
    )
    ...
    assert len(data["data"]["ongoing_decisions"]) == 1
    assert data["data"]["ongoing_decisions"][0]["title"] == "Ongoing Decision"
    
    # Completed should not be in ongoing
    ongoing_titles = [d["title"] for d in data["data"]["ongoing_decisions"]]
...
async def test_today_todays_decisions(client: AsyncClient, auth_headers: dict):
    ...
    assert len(data["data"]["todays_decisions"]) == 1
    assert data["data"]["todays_decisions"][0]["title"] == "Today's Decision"
...
async def test_today_todays_memos(client: AsyncClient, auth_headers: dict):
    ...
    assert len(data["data"]["todays_memos"]) == 1
    assert data["data"]["todays_memos"][0]["content"] == "Today's Memo"
...
async def test_today_with_custom_date(client: AsyncClient, auth_headers: dict):
    ...
    assert data["data"]["date"] == "2026-02-15"
    assert len(data["data"]["ongoing_decisions"]) == 1
    assert len(data["data"]["todays_decisions"]) == 1
    assert len(data["data"]["todays_memos"]) == 1
...
async def test_today_deleted_items_not_shown(client: AsyncClient, auth_headers: dict):
    ...
    assert len(data["data"]["todays_decisions"]) == 0
    assert len(data["data"]["todays_memos"]) == 0
